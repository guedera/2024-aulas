...
class SeguidorNode(Node, Odom):
    ...
    def __init__(self):
        ...
        self.goal_points = [(1.25, 2.7), (2.9, 2.7)]
        self.current_goal_index = 0
        self.goal_tolerance = 0.2

        # Flags for conditions
        self.all_creepers_detected = False
        ...
        # Subscriber to mobilenet_detection
    ...
    def image_callback(self, msg):
        if self.running:
            ...
            aruco_image, aruco_results = self.aruco_detector.detectaAruco(imagem_cheia)
    ...
    def segue(self):
        ...
        if self.aruco150 and self.girou == False:
    ...
    def mobilenet_callback(self, msg):
        for detection in msg.deteccoes:
            if detection.classe in ['bicycle', 'dog', 'cat']:
                if self.creepers[detection.classe] == (0, 0):

                    self.creepers[detection.classe] = (self.x, self.y)
                    self.get_logger().info(
                        f"{detection.classe} detectado ({self.x}, {self.y})"
                    )
        creeper_positions = [self.creepers[obj] for obj in ['bicycle', 'dog', 'cat']]
        if all(pos != (0, 0) for pos in creeper_positions):
            self.all_creepers_detected = True
            self.get_logger().info('Todos os creepers foram detectados!')
